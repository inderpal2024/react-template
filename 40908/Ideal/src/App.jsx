      ...
      setStocks(prevStocks => {
        ...
        for (let sector in updatedStocks) {
          updatedStocks[sector] = updatedStocks[sector].map(stock => ({
            ...stock,
            percentageChange: (Math.floor(Math.random() * 40) + 1) / 4 - 5,
          }));
        }
  ...
  return (
    <div className="p-4 max-w-3xl mx-auto">
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-0 sm:flex sm:space-x-4 sm:items-end">
                ...
                {stocks[sector].map((stock, idx) => (
                  <li key={idx} className={cn(
                    ...
                    <span className={cn(
                      ...
                      {stock.percentageChange === 0 ? '0.00%' : `${stock.percentageChange >= 0 ? '' : '('}`
                          + `${Math.abs(stock.percentageChange).toFixed(2)}%${stock.percentageChange >= 0 ? '' : ')'}`}
                    </span>
                  </li>
                ))}
              </ul>
